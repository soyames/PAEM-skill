...
| 1.0.0 | 1.1.0 | Managed saves add three optional fields: `parent_checkpoint_id`, `repository_state` (worktree root, git dir, HEAD, branch, and a digest over the index and dirty-file bytes), and `verification_basis`. No required field changed shape or meaning. A save also refreshes `timestamp`, `branch` and `commit_hash` from the observed repository, so those describe the generation rather than the session that requested it. | Nothing required. 1.0.0 records stay valid and readable (`scripts/paem_checkpoint.py check` reports a pre-writer `.paem/` as `legacy` rather than invalid). A hand-written 1.0.0 record is upgraded on the next managed save: review the existing `.paem/`, then publish with `--adopt-legacy`, which copies the previous `latest_checkpoint.json` and `resume_prompt.md` to `.paem/legacy/<random-id>/` before writing. No tool may rewrite an old record in place. |
