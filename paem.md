...
Execution should be continuous, verifiable, and recoverable.

"Verifiable" is the load-bearing word: a checkpoint that cannot be checked
against the repository is a note, not evidence. Publishing through
`scripts/paem_checkpoint.py` keeps the selected generation and its repository
binding checkable; `scripts/paem_checkpoint.py check` reports the result. Nothing
in PAEM makes an agent's *claims* verifiable - `verification_basis: self_reported`
says so explicitly on every managed record.
